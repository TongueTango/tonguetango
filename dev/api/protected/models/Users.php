...
class Users extends BaseUsers
{
    ...
    {
	    //|
	    //|   This will check to see if a given user already has a contact with a specific
...
//	        print "NOT SET:"; print_r($this);
    ...
    public function get_tongue_tango_friends($friends)
    {
    	...
    	if( is_array($friends) ) 
    	{
			...
			if ( $fb_ids != '' ) {
		        ...
		        $sql = "
		            ...
		            LEFT JOIN users u
		            ON u.person_id = p.id
		            WHERE (p.facebook_id IN($fb_ids) AND u.id NOT IN (SELECT blocked_user_id FROM blocks WHERE(user_id=".$this->id." AND blocked = 1)))
        ...
        file_put_contents("/tmp/brian.txt", print_r($results,1));
        $tt_friends	= array();
        $fb_friends	= array();
        foreach($results as $result)
        {
            ...
            if ($tt_friend["user_id"] > 0) 
            {
				...
				$this->checkAndCreateContact(intval($result["user_id"]));
			} else {
        ...
        $sql = "
        	...
        	LEFT JOIN people p
        		ON p.id = u.person_id
        	WHERE ((user_id =".$this->id." OR contact_user_id =".$this->id.") AND u.id NOT IN (SELECT blocked_user_id FROM blocks WHERE(user_id=".$this->id." AND blocked = 1)))
        ";
