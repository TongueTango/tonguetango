...
class Controller extends CController
{
    ...
    */
    ...
    {   
        ...
        { 
            $this->_sendResponse(400, array("code"=>0, 'message'=>$event->exception->getMessage(),'line'=>$event->exception->getLine(),'file'=>$event->exception->getFile()));
        }
        elseif($event instanceof CErrorEvent)
        {
            $this->_sendResponse(400, array("code"=>0, 'message'=>$event->message,'line'=>$event->line,'file'=>$event->file));
        }
    ...
    protected function beforeAction($action)
    {
         ...
         if (!$auth_token) {
         	  $auth_token = Yii::app()->request->getPost('token');
         }

         if (!$auth_token && isset($_REQUEST["token"])) {
     ...
     protected function _process_input()
     {
        ...
        switch($content_type) {
            ...
            case 'multipart/form-data':
                $skip_post = true;
                $post_body = $_REQUEST['body'];
                $this->_process_multipart($post_body);
                break;
            default:
                $post_body = file_get_contents('php://input');
...
//      	Kohana::$log->add(Log::DEBUG, '_set_input');
     ...
     public function auth_user($auth_token, $controller_exceptions = array())
     {
		if (!$auth_token ) 
		{
			...
			{
				$this->error(array('code'=>'401','message'=>'Invalid Auth Token (1)!'), 401);
			}
		} else {
			$this->_device = Devices::model()->find('auth_token=\'' . $auth_token . '\'');
			self::$device	= $this->_device;
			// if( !$this->_device ) {
				//$this->error(array('code'=>'401','message'=>'Invalid Auth Token! (2)'), 401);
			// }
